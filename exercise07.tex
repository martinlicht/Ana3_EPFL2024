...
We solve each integral seperately. Note that we apply integration by parts in between steps. 
    \begin{align*}
        \int_{0}^{1}\int_{0}^{1 - x_3}\int_{0}^{1 - x_2 - x_3} 2x_1x_2 dx_1 dx_2 dx_3 &= \int_{0}^{1}\int_{0}^{1 - x_3} (1-x_2-x_3)^2 x_2 dx_2 dx_3\\
...
\begin{solution}     
    \begin{itemize}
     \item $$\Phi: \Omega:= [0,2\pi)\times[0,\pi) \mapsto S: (\theta,\phi)\mapsto (\cos\theta\sin\phi,\sin\theta\sin\phi,\cos\phi) $$
