...
$x_2$ is an odd function therefore the last two integrals are 0. The evaluate the first two integrals,
...
\begin{align*}
    ...
    =&
    \sum_{k = 0}^5 \binom{5}{k} (-1)^{5-k} \left[\frac{x^{2k+1}}{2k+1}\right]_{-1}^1 + \sum_{k = 0}^5 \binom{5}{k}\right)\left[\frac{x^{2k+1}}{2k+1}\right]_{-1}^1\\
    =&
...
\begin{solution}
    ...
    $$
        \int_{\Gamma} \vec{F} \cdot \; d\vec{l}
        =
        \int_{\Gamma\cup \Delta} \vec{F} \cdot \; d\vec{l}
        -
        \int_{\Delta} \vec{F}\cdot \;d \vec{l}
    $$
