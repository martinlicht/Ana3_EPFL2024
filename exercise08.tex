...
\end{exercise}
\begin{solution}     
    We conduct this in several steps.
    \begin{itemize}
    \item 
    One possible parameterisation of the surface $S$ reads:
    \begin{gather*}
        \Omega := [0,2\pi)\times[0,\tfrac{\pi}{2}),
        \\
        \Phi: \Omega \rightarrow S, \quad (\theta,\phi)\mapsto
        (
            \cos\theta\cos\phi,
            \sin\theta\cos\phi,
            \sin\phi
        ...
        .
    \end{gather*}
    We think of this as a parameterization of the upper hemisphere. 
    The first variable $\theta$ parameterizes the equator-like lines (latitude) and the second variable paramterizes the longitude (how far up north, starting at the equator).
    ...
    \begin{gather*}
        ...
        ,
        \\
        \partial_{\theta} \Phi \times \partial_{\phi} \Phi
        =
        \begin{pmatrix}
            \cos\theta \cos^2\phi
            \\
    ...
    \begin{align*}
        \left\| \partial_{\theta} \Phi \times \partial_{\phi} \Phi \right\|
        &=
        \sqrt{
            \cos^{2}\theta \cos^{4}\phi
            +
            \sin^{2}\theta \cos^{4}\phi
            +
            \sin^{2}\phi \cos^{2}\phi
        }
        \\
    ...
    \begin{gather*}
        \vec{n} 
        = 
        \frac{\partial_{\theta}\Phi\times \partial_{\phi}\Phi}{\|\partial_{\theta}\Phi\times \partial_{\phi}\Phi\|} 
        = 
        \begin{pmatrix}    
            \cos\theta\cos\phi 
            \\ 
        ...
        \end{pmatrix}
    \end{gather*}
    According to the right-hand rule, when walking along the boundary with our right hand pointing into the first direction and our left-hand pointing into the interior of the surface, the resulting normal points will be the one pointing away from the origin.
    
    \begin{figure}
    \centering
    \includegraphics[scale=0.20]{Northern_Hemisphere.png}
    \caption{The first variable eastwards. The second variable points up north. The resulting normal is pointing away from the origin. Remark: if we only consider the upper hemisphere surface, then there is no outside or inside pointing normal. Indeed, if you cut open the Earth and only consider the upper hemisphere, then there would be inside left.}
    \end{figure}
    
    \item 
    We first parameterize the boundary of the parameter space $\Omega$ 
    \begin{gather*}
        \gamma : [0,2\pi) \mapsto \partial \Omega, \quad \theta \mapsto \left( \theta, \frac{\pi}{2} \right),
    \end{gather*}
	which gives us the parameterization of the boundary as follows:
    \begin{gather*}
        \Phi\circ \gamma :[0,2\pi) \mapsto \partial C, \quad \theta \mapsto(\cos\theta,\sin\theta,0).
    \end{gather*}
    ...
    \begin{align*}
        ...
        \\&
        = 
        2\pi \int_0^{\frac{\pi}{2}} 2\cos\phi\sin\phi \ d\phi d\theta
        = 
        2\pi \int_0^{\frac{\pi}{2}} \sin(2\phi) \ d\phi d\theta
        = 
        2\pi \left[ \frac{1}{2}\cos2\phi \right]_0^{\frac{\pi}{2}}
        = 
        2\pi
        .
    \end{align*}   
    ...
    \begin{align*}
        ...
        &
        = 
        \int_0^{2\pi} \vec{F}(\Phi\circ \gamma)\cdot \frac{d(\Phi\circ \gamma)}{d\theta}  d \theta
        \\&
        = 
        \int_0^{2\pi} 
...
\begin{solution}     
    ...
    In a similar fashion as Exercise 1 Subquestion 2 we obtain the following normal vector:
		\begin{gather*}\vec{n} = \begin{pmatrix} x_1\\x_2\\0 \end{pmatrix}\end{gather*}
    \item 
	\begin{align*}
...
\begin{exercise}[Green's theorem]
    ...
    \begin{itemize}
        ...
        \begin{gather*} \oint_C \vec F dl \end{gather*}
...
\end{exercise}
\begin{solution}     
    This is a quarter of a globe. We parameterize this surface by 
    The surface is parameterized by 
    \begin{gather*}
        \Omega := [0,\tfrac{\pi}{2}] \times [0,\pi],
        \\
        \Phi : \Omega \mapsto S, \quad (\theta, \phi) \mapsto \left( \cos\phi, \cos\theta\sin\phi, \sin\theta\sin\phi \right)
    \end{gather*}
    For the boundary parameterisation, we walk from the North pole to South pole along one side and then back from the South pole to the North pole along the other side. 
    The parameterization is given by the two curves
    \begin{align*}
        &\gamma_1 : \left[0,\pi\right] \to \bbR^3, \quad \phi \mapsto (       \cos \phi, \sin \phi,               0 ),
        \\
        &\gamma_2 : \left[0,\pi\right] \to \bbR^3, \quad \phi \mapsto ( \cos (\pi-\phi),         0, \sin (\pi-\phi) ).
    \end{align*}
    The derivatives of those two curves are 
    \begin{align*}
        &\dot\gamma_{1} : \left[0,\pi\right] \to \bbR^3, \quad \phi \mapsto ( -\sin \phi, \cos \phi, 0 ),
        \\
        &\dot\gamma_{2} : \left[0,\pi\right] \to \bbR^3, \quad \phi \mapsto ( \sin(\pi-\phi), 0, -\cos(\pi-\phi) ).
    \end{align*}
    Equipped with that, we use the vector field as in the problem statement, 
    \begin{align*}
        \vec F( x_1, x_2, x_3 ) = \left( x_1 x_2, x_2 x_3, x_1 x_3 \right),
    \end{align*}
    and, after renaming the paramter variable from $\phi$ to $t$, compute 
    \begin{align*}
        \iint_{S} \operatorname{curl} \vec F \;d\sigma 
        &
        =  
        \iint_{\gamma_{1}} \left( \vec F\circ\gamma_{1} \right) \cdot \dot\gamma_{1} \;d\sigma 
        +
        \iint_{\gamma_{2}} \left( \vec F\circ\gamma_{2} \right) \cdot \dot\gamma_{2} \;d\sigma 
        \\&
        =  
        \int_0^{\pi} \begin{pmatrix} \cos t \sin t\\ 0 \\ 0\end{pmatrix}
        \cdot 
        \begin{pmatrix} -\sin t \\ \cos t \\ 0 \end{pmatrix} 
        dt 
        +
        \int_0^{\pi} 
        \begin{pmatrix} 0\\ 0 \\ \sin(\pi-t) \cos (\pi-t)\end{pmatrix}
        \cdot 
        \begin{pmatrix} \sin(\pi-t) \\ 0 \\ -\cos(\pi-t) \end{pmatrix}
        dt
        \\&
        =  
        \int_0^{\pi} \begin{pmatrix} \cos t \sin t\\ 0 \\ 0\end{pmatrix}
        ...
        +
        \int_0^{\pi} 
        \begin{pmatrix} 0\\ 0 \\ \sin(t) \cos (t)\end{pmatrix}
        \cdot 
        ...
        = 
        -
        \left[ \tfrac{1}{3} \sin^{3}(t) \right]_{t=0}^{t=\pi}
        -
        \left[ \tfrac{1}{3} \cos^{3}(t) \right]_{t=0}^{t=\pi}
        = 
        -
        \left[ \tfrac{1}{3} \cos^{3}(t) \right]_{t=0}^{t=\pi}
        = 
