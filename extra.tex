...
\begin{exercise}
    ...
    \[
        f(t) + \lambda \int_{0}^{+\infty} e^{-|\tau|} f( t - \tau ) \ d\tau = g(t).
    \]
\end{exercise}
\begin{solution}
    We observe that this is a convolutional integral equation 
    \[
        f(t) + \lambda ( K \star f )(t) = g(t)
    \]
    with 
    \[
        % \lambda = 3, \qquad 
    ...
    \[
        ...
        \quad 
        \hat g(\alpha) = \sqrt{ \frac{2}{\pi} } \frac{1}{1+\alpha^2}
    \]
    We Fourier transform the equation, 
    \[
        \hat f(t) + \lambda \sqrt{2\pi} \hat K(\alpha) \cdot \hat f(\alpha) = \hat g(\alpha),
    \]
    isolate $\hat f$, and find 
    \[
        \hat f(\alpha) = \frac{ \hat g(\alpha) }{ 1 + \lambda \sqrt{2\pi} \hat K(\alpha) },
    \]
    ...
    \begin{align*}
        ...
        &
        = 
        \frac{ \hat g(\alpha) }{ 1 + \lambda \frac{1}{ 1 + i \alpha } }
        \\&
        = 
        \frac{ \hat g(\alpha) ( 1 + i \alpha ) }{ ( 1 + i \alpha ) + \lambda }
        =
        \sqrt{ \frac{2}{\pi} } 
    ...
    \[
        \hat u(\alpha) = \frac{ 1 }{ ( 1 + \lambda ) + i \alpha }
        \quad \implies \quad 
             u(t)      = \sqrt{2\pi} \begin{cases} e^{-( 1 + \lambda )t} & t > 0 \\ 0 & \text{ otherwise} \end{cases} 
        .
    \]
    Using the modulation identity, we find 
    \[
        \hat v(\alpha) = \frac{ 1 }{ 1 - i \alpha }
        \quad \implies \quad 
             v(t)      = \sqrt{2\pi} \begin{cases} e^{t} & t < 0 \\ 0 & \text{ otherwise} \end{cases} 
        .
    \]
    Hence 
    \[
        \hat f(\alpha)  = \hat u(\alpha) \hat v(\alpha) 
    \]
    which means 
    \[
        f(t) = \frac{1}{\sqrt{2\pi}} ( u \star v )(t)
    \]
    Explicitly, 
    \begin{align*}
        f(t) 
        = 
        \int_{0}^{+\infty} e^{-(1+\lambda)\tau} v( t - \tau ) \ d\tau
        &
        = 
        \int_{t}^{+\infty} e^{-(1+\lambda)\tau} v( t - \tau ) \ d\tau
        \\&
        = 
        e^t
